import { useEffect, useRef } from "react";
import { personalInfo } from "../data/portfolioData";
import "./Hero.css";

// rotates through these in the typewriter
const roles = [
  "Competitive Programmer",
  "Full Stack Developer",
  "Problem Solver",
  "React Developer",
  "CP Trainer @ IIUC",
];

export default function Hero() {
  const typingEl = useRef(null);

  useEffect(() => {
    let i = 0, j = 0, deleting = false;
    const el = typingEl.current;
    if (!el) return;

    const tick = () => {
      const word = roles[i];
      if (!deleting) {
        el.textContent = word.slice(0, ++j);
        if (j === word.length) {
          deleting = true;
          setTimeout(tick, 1600);
          return;
        }
      } else {
        el.textContent = word.slice(0, --j);
        if (j === 0) {
          deleting = false;
          i = (i + 1) % roles.length;
        }
      }
      setTimeout(tick, deleting ? 40 : 80);
    };

    tick();
  }, []);

  return (
    <section id="hero" className="hero-section">
      <div className="blob blob-1" />
      <div className="blob blob-2" />
      <div className="blob blob-3" />

      <div className="hero-content container">
        <div className="hero-text fade-up">
          <div className="hero-greeting">👋 Hello, I'm</div>
          <h1 className="hero-name">{personalInfo.name}</h1>

          <h2 className="hero-typewriter">
            <span ref={typingEl} />
            <span className="cursor">|</span>
          </h2>

          <p className="hero-location">📍 {personalInfo.location}</p>

          <p className="hero-desc">
            CSE student at IIUC. I spend most of my time solving algorithm problems
            and building web apps. Solved <strong>1000+</strong> problems on Codeforces.
          </p>

          <div className="hero-btns">
            <a href="#contact" className="btn btn-primary">✉️ Contact Me</a>
            <a href={personalInfo.cvLink} download className="btn btn-outline">⬇️ Download CV</a>
          </div>

          <div className="hero-socials">
            <a href={personalInfo.github} target="_blank" rel="noreferrer" className="social-link" aria-label="GitHub">
              <svg viewBox="0 0 24 24" fill="currentColor" width="22" height="22">
                <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/>
              </svg>
            </a>
            <a href={personalInfo.linkedin} target="_blank" rel="noreferrer" className="social-link" aria-label="LinkedIn">
              <svg viewBox="0 0 24 24" fill="currentColor" width="22" height="22">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
              </svg>
            </a>
            <a href={`mailto:${personalInfo.email}`} className="social-link" aria-label="Email">
              <svg viewBox="0 0 24 24" fill="currentColor" width="22" height="22">
                <path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/>
              </svg>
            </a>
          </div>
        </div>

        <div className="hero-visual fade-in">
          <div className="code-card">
            <div className="code-header">
              <span className="dot red" />
              <span className="dot yellow" />
              <span className="dot green" />
              <span className="code-filename">musabbir.js</span>
            </div>
            <pre className="code-body">
{`const musabbir = {
  role: "CSE Student @ IIUC",
  skills: ["C++", "React", "Node"],
  cp: {
    codeforces: "Turag9",
    solved: "1000+",
    rating: "Active"
  },
  goal: "Build impactful
         software 🚀"
};`}
            </pre>
          </div>

          <div className="hero-badge">
            <span>🏆</span>
            <span>ICPC Contestant</span>
          </div>
          <div className="hero-badge badge-2">
            <span>⭐</span>
            <span>1000+ Problems Solved</span>
          </div>
        </div>
      </div>

      <div className="scroll-indicator">
        <div className="scroll-dot" />
      </div>
    </section>
  );
}
